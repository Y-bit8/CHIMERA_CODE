...
class SubExtractor(nn.Module):
    def __init__(self, hidden_dim, num_clusters, residual=False, chirality_alpha=None):
        super().__init__()
        self.chirality_alpha = chirality_alpha
    ...
    def forward(self, x, batch, chirality_feas, use_chiral= False):
        ...
        if use_chiral ==True:
            ...
            alpha = self.chirality_alpha if self.chirality_alpha is not None else float(os.getenv("CHIMERA_CHIRALITY_ALPHA", "2.0"))
            alpha = float(alpha)
...
class Transformer(torch.nn.Module):
    def __init__(self, dataset, num_layers, hidden, num_patterns=10, cfg=None):
        ...
        self.chirality_alpha = model_cfg.get("chirality_alpha", None)
        ...
        self.pool1 = SubExtractor(2*hidden, num_patterns, False, chirality_alpha=self.chirality_alpha)
        self.pool2 = SubExtractor(2*hidden, num_patterns, False, chirality_alpha=self.chirality_alpha)
